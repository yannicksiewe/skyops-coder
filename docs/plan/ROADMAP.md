...
| 1 | CI/CD on the office server | self-hosted GitHub Actions runner on the VM; `ci.yml` (lint + tests), `cd.yml` (deploy on merge) | done |
| 2 | AI code review | `review.yml` posts a review on every PR using the local model; architecture review on request | done |
| 3 | AI coding agent | issue labelled `ai-task` -> agent implements it with aider -> opens a PR | done |
| 4 | Monitoring | Prometheus + Grafana + node/GPU exporters + vLLM metrics, dashboards in the repo | done |
| 5 | Backup & recovery | nightly encrypted backup of state (keys, UI data, CA, adapters), tested restore | done |
| 6 | Model research | measured comparison of what fits on 8 GB cards, incl. Qwen3.5-9B trial; GLM assessment | done |
