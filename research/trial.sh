...
set -euo pipefail
MODEL="$1"; GPU="$2"; LABEL="$3"; shift 3; PORT="${PORT:-8009}"
...
pkill -f "port $PORT --api-key" 2>/dev/null || true; sleep 3
nohup env CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES="$GPU" VLLM_USE_FLASHINFER_SAMPLER=0 \
  "$HOME/vllm/.venv/bin/vllm" serve "$MODEL" --host 127.0.0.1 --port "$PORT" --api-key "$VLLM_API_KEY" --served-model-name trial \
  --max-model-len 8192 --max-num-seqs 2 --gpu-memory-utilization 0.95 "$@" > "$LOG" 2>&1 < /dev/null &
up=0
for _ in $(seq 1 90); do
  if curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer $VLLM_API_KEY" localhost:"$PORT"/v1/models | grep -q 200; then up=1; break; fi
...
if [ "$up" = 0 ]; then
  ...
  pkill -f "port $PORT --api-key" 2>/dev/null || true; exit 1
fi
...
python3 bench.py --base "http://127.0.0.1:$PORT/v1" --model trial --label "$LABEL" --no-think 2>&1 | tail -9
pkill -f "port $PORT --api-key" 2>/dev/null || true; sleep 3
